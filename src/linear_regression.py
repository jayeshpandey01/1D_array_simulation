...
class LinearRegressionModel(nn.Module):
    def __init__(self, input_size, hidden_neurons=256, dropout_rate=0.15):
        ...
        self.network = nn.Sequential(
            ...
            nn.BatchNorm1d(hidden_neurons // 4),
            nn.LeakyReLU(0.1),
...
class LinearRegressionDataModule:
    ...
    def get_dataloaders(self, batch_size=32, test_size=0.2, val_size=0.1):
        """Create DataLoaders for train, validation, and test sets"""
        X_train, Y_train, X_val, Y_val, X_test, Y_test = self.split_data(test_size, val_size)
        
        train_dataset = linear_regression_dataset(X_train, Y_train)
        val_dataset = linear_regression_dataset(X_val, Y_val)
